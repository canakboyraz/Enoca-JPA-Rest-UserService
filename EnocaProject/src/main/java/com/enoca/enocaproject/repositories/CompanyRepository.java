...
public interface CompanyRepository extends JpaRepository<Company, Long> {
    @Query(value = "select p.* from person as p inner join person_person_company ppc on p.person_id = ppc.person_person_id where ppc.person_company_company_id =?1",nativeQuery = true)
